...
disabled, otherwise a warning will be printed and the GIL is re-enabled at
runtime when a module that does not support the GIL is imported. To force
...
Many Python extension modules are not thread-safe in the free-threaded build
eas of mid-2024. Up until now, the GIL has added implicit locking around any
...
the GIL, attempting to parallelize many workflows using the python threading
module will not produce any speedups, so thread safety issues that are possible
even with the GIL are not hit often since users do not make use of it as much
as other parallelization strategies.
...
support the free-threaded build, want to encourage people to test it, we
...
We are generally assuming users will not do pathological things like resizing
an array while another thread is writing to it. Eventually we will need to add
locking around data structures to avoid issues like this, but in this early
stage of porting we are not focusing on adding locking on every operation
exposed to users on all mutable data structures.
...
updated by another thread while it is running is no longer thread safe. In
particular, C code returning "borrowed" references to Python objects in mutable
containers like lists may introduce thread safety issues. A borrwed reference
...
thread and the C-level implementation of a python function can only be called
on one thread. Another example: accessing the arguments of a
python function is thread safe no matter what C API constructs are used and no
matter whether the reference is borrowed or owned because two threads can't
simultaneously call the same function with the same arguments from the same
