...
development workflows that have not been updated to accommadate the
free-threading build. We also provide suggested fixes. Please send in pull
...
Github](https://github.com/cython/cython) or using the nightly wheel:
...
Freethreading support in Cython relies CPython internals. In particular, cython
will in some generate code that in some situations includes an internal CPython
header, `pycore_frame.h`. This header, in turn, includes another header
`pycore_code.h` that makes use of `static_assert`, a construct defined in the
C11 standard. This is problematic for a compiler assuming compliance with
C99. 
...
Note that C17 is for the most part a subset of C11, so requiring C11 support may
be more enerous than C17. The CPython codebase currently assumes a compiler with
at least partial C11 support, and we have found for all major compilers
(including MSVC) that updating the assumed C standard is not a big problem as of
mid-2024. There are some C11 features that MSVC and other compilers are missing,
so please open an issue if this ends up being a big problem for your project or
a subset of your userbase.
